...
module.exports = async (req, res) => {
  ...
  if (!authHeader?.startsWith('Bearer ')) {
    console.warn(
      `[AUTH FAIL] ${new Date().toISOString()} | ${ip} | delete-expert | missing token`
    )
  ...
  if (authError || !user) {
    console.warn(
      `[AUTH FAIL] ${new Date().toISOString()} | ${ip} | delete-expert | invalid token`
    )
  ...
  if (profile?.role !== 'admin') {
    console.warn(
      `[AUTH FAIL] ${new Date().toISOString()} | ${ip} | delete-expert | insufficient role: ${profile?.role}`
    )
  ...
  try {
    ...
    const { error: deleteError } =
      await supabaseAdmin.auth.admin.deleteUser(expertId)
    if (deleteError) {
      return res
        .status(400)
        .json({ error: 'Failed to delete expert. Please try again.' })
    }

    return res.status(200).json({
      success: true,
      message: `Expert ${expertProfile.email} has been deleted`,
    })
  } catch (err) {
