...
module.exports = async (req, res) => {
  ...
  if (!authHeader?.startsWith('Bearer ')) {
    console.warn(
      `[AUTH FAIL] ${new Date().toISOString()} | ${ip} | notify-assigned-expert | missing token`
    )
  ...
  if (authError || !user) {
    console.warn(
      `[AUTH FAIL] ${new Date().toISOString()} | ${ip} | notify-assigned-expert | invalid token`
    )
  ...
  if (!callerProfile || !['admin', 'staff'].includes(callerProfile.role)) {
    console.warn(
      `[AUTH FAIL] ${new Date().toISOString()} | ${ip} | notify-assigned-expert | insufficient role: ${callerProfile?.role}`
    )
  ...
  if (!expertId || !caseTitle) {
    return res
      .status(400)
      .json({ error: 'Expert ID and case title are required' })
  }
  ...
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: `Notification sent to ${expert.email}` })
  } catch (err) {
