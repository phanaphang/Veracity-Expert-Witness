...
module.exports = async (req, res) => {
  ...
  if (!authHeader?.startsWith('Bearer ')) {
    console.warn(
      `[AUTH FAIL] ${new Date().toISOString()} | ${ip} | notify-message | missing token`
    )
  ...
  if (authError || !user) {
    console.warn(
      `[AUTH FAIL] ${new Date().toISOString()} | ${ip} | notify-message | invalid token`
    )
  ...
  if (!recipientId || !senderName || !messagePreview) {
    return res.status(400).json({
      error: 'recipientId, senderName, and messagePreview are required',
    })
  }
