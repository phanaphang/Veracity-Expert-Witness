...
module.exports = async (req, res) => {
  ...
  if (!authHeader?.startsWith('Bearer ')) {
    console.warn(
      `[AUTH FAIL] ${new Date().toISOString()} | ${ip} | training/report-writing-certificate-issued | missing token`
    );
  ...
  if (authError || !user) {
    console.warn(
      `[AUTH FAIL] ${new Date().toISOString()} | ${ip} | training/report-writing-certificate-issued | invalid token`
    );
  ...
  if (!profile || !['expert', 'admin', 'staff'].includes(profile.role)) {
    console.warn(
      `[AUTH FAIL] ${new Date().toISOString()} | ${ip} | training/report-writing-certificate-issued | insufficient role`
    );
      ...
      <div style="background:#ffffff;padding:32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px;">
        ...
        <table style="border-collapse:collapse;width:100%;margin:24px 0;">
          ...
          <tr>
            ...
            <td style="padding:10px 12px;border:1px solid #e5e7eb;color:#374151;">${MODULE_TITLE} (~60 min)</td>
          </tr>
        </table>
  ...
  try {
    ...
    console.error('report-writing-certificate-issued error:', err.message);
