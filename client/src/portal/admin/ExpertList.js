...
export default function ExpertList() {
  ...
  useEffect(() => {
    Promise.all([
      supabase
        .from('profiles')
        .select('*, expert_specialties(specialty_id, specialties(name))')
        .eq('role', 'expert')
        .order('created_at', { ascending: false }),
      getCached('specialties', () =>
        supabase
          .from('specialties')
          .select('*')
          .order('name')
  ...
  const parents = useMemo(
    () => specialties.filter((s) => !s.parent_id),
    [specialties]
  )
  const subsOf = (parentId) =>
    specialties.filter((s) => s.parent_id === parentId).map((s) => s.id)
  ...
  const filtered = useMemo(
    () =>
      experts.filter((exp) => {
        const name =
          `${exp.first_name || ''} ${exp.last_name || ''} ${exp.email || ''}`.toLowerCase()
        if (search && !name.includes(search.toLowerCase())) return false
        if (filterAvailability && exp.availability !== filterAvailability)
          return false
        if (filterSpecialties.size > 0) {
          const matchesAny = [...filterSpecialties].some((filterId) => {
            ...
            if (spec && !spec.parent_id) {
              ...
              return exp.expert_specialties?.some((es) =>
                ids.includes(es.specialty_id)
              )
            }
            return exp.expert_specialties?.some(
              (es) => es.specialty_id === filterId
            )
          })
  ...
  useEffect(() => {
    setPage(1)
  }, [search, filterSpecialties, filterAvailability, filterTag])
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      const exportExperts = [...filtered].sort((a, b) =>
      ...
      const [docsRes, eduRes, expRes, credRes, testRes] = await Promise.all([
        ...
        supabase
          .from('prior_testimony')
          .select('*')
          .in('expert_id', expertIds)
          .order('date_of_testimony', { ascending: false }),
      ])
      ...
      const rows = exportExperts.map((exp) => {
        ...
        return {
          ...
          Email: exp.email || '',
          Phone: exp.phone || '',
          Specialties: (exp.expert_specialties || [])
            .map((es) => es.specialties?.name)
            .filter(Boolean)
            .join(', '),
          Tags: (exp.tags || []).join(', '),
          ...
          Education: expertEdu
            .map((e) =>
              `${e.degree || ''} ${e.field_of_study ? 'in ' + e.field_of_study : ''} — ${e.institution || ''}${e.end_year ? ' (' + e.end_year + ')' : ''}`.trim()
            )
            .join('; '),
          'Work Experience': expertExp
            .map((e) =>
              `${e.title || ''} at ${e.organization || ''}${e.is_current ? ' (Current)' : ''}`.trim()
            )
            .join('; '),
          Credentials: expertCred
            .map((c) =>
              `${c.name || ''}${c.issuing_body ? ' — ' + c.issuing_body : ''} (${c.credential_type || ''})`.trim()
            )
            .join('; '),
          'Prior Testimony': expertTest
            .map((t) =>
              `${t.case_name || ''}${t.court ? ' — ' + t.court : ''}${t.retained_by ? ' [' + t.retained_by + ']' : ''}`.trim()
            )
            .join('; '),
      ...
      for (const exp of exportExperts) {
        ...
        if (expertDocs.length > 0) {
          ...
          if (expertDocs.length === 1 && signedUrlMap[expertDocs[0].id]) {
            ws[cellRef].l = {
              Target: signedUrlMap[expertDocs[0].id],
              Tooltip: expertDocs[0].file_name,
            }
          }
        }
        rowIndex++
      ...
      XLSX.writeFile(
        wb,
        `Experts_${new Date().toISOString().split('T')[0]}.xlsx`
      )
      ...
      <div className="portal-stats" style={{ marginBottom: 24 }}>
        <button
          ...
          style={{
            ...
            borderColor:
              filterSpecialties.size === 0 ? 'var(--color-accent)' : undefined,
          }}
          ...
          return (
            <button
              ...
              style={{
                cursor: 'pointer',
                transition: 'border-color 0.15s',
                borderColor: filterSpecialties.has(parent.id)
                  ? 'var(--color-accent)'
                  : undefined,
              }}
          ...
          >
            <span style={{ flex: 1 }}>
              {filterSpecialties.size === 0
                ? 'All Specialties'
                : `Specialties (${filterSpecialties.size})`}
            </span>
            <span
              style={{ fontSize: '0.7rem', color: 'var(--color-gray-400)' }}
            >
              {filterPanelOpen ? '▲' : '▼'}
            </span>
          </button>
                    ...
                    >
                      ...
                      {subs.length > 0 && (
                        <button
                          ...
                          title={
                            isExpanded
                              ? 'Collapse subspecialties'
                              : 'Expand subspecialties'
                          }
                          aria-label={
                            isExpanded
                              ? `Collapse ${parent.name} subspecialties`
                              : `Expand ${parent.name} subspecialties`
                          }
                        >
                          {isExpanded ? '▲' : '▼'}
                        </button>
          ...
          {[...filterSpecialties].map((id) => {
            ...
            return (
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() => toggleFilterSpecialty(id)}
                  style={{
                    background: 'none',
                    border: 'none',
                    cursor: 'pointer',
                  ...
                  aria-label={`Remove ${spec.name} filter`}
                >
                  ×
                </button>
              </span>
            ...
            <tbody>
              {paginatedExperts.map((exp) => (
                <tr key={exp.id}>
                  ...
                  <td>
                    <span
                      className={`portal-badge portal-badge--${exp.availability || 'pending'}`}
                    >
                  ...
                  <td>
                    <span
                      className={`portal-badge portal-badge--${exp.onboarded_at ? 'accepted' : 'pending'}`}
                    >
      ...
      {exportConfirm && (
        ...
        >
          ...
          >
            ...
            >
              Are you sure you want to export the Experts database to an Excel
              file?
            </p>
            <div
        ...
        >
          ...
          >
            ...
            >
              ...
              ? This will remove their account, profile, documents, and all
