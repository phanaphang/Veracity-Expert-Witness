...
export default function TrainingReport() {
  ...
  useEffect(() => {
    const load = async () => {
      const [{ data: experts }, { data: foundationsProgress }, { data: admissibilityProgress }, { data: reportWritingProgress }] = await Promise.all([
        supabase
          .from('profiles')
  ...
  const getStatus = (row) => {
    ...
    return row.reportWriting.status;
  };
  ...
  const statusLabels = activeTab === 'foundations' ? STATUS_LABELS : activeTab === 'admissibility' ? AD_STATUS_LABELS : RW_STATUS_LABELS;
