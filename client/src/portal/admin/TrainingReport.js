...
export default function TrainingReport() {
  ...
  useEffect(() => {
    const load = async () => {
      const [{ data: experts }, { data: foundationsProgress }, { data: admissibilityProgress }, { data: reportWritingProgress }, { data: depositionProgress }, { data: trialTestimonyProgress }] = await Promise.all([
        supabase
          .from('profiles')
  ...
  const statusLabels = activeTab === 'foundations' ? STATUS_LABELS : activeTab === 'admissibility' ? AD_STATUS_LABELS : activeTab === 'report-writing' ? RW_STATUS_LABELS : activeTab === 'trial-testimony' ? TT_STATUS_LABELS : DP_STATUS_LABELS;
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24, borderBottom: '2px solid var(--color-gray-100)', paddingBottom: 0 }}>
        ...
        <button onClick={() => handleTabChange('trial-testimony')} style={tabStyle('trial-testimony')}>
          Trial Testimony
        </button>
      </div>
            ...
            <React.Fragment key={expert.id}>
              ...
              >
                ...
                <td style={{ minWidth: 130 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ flex: 1, height: 6, background: 'var(--color-gray-100)', borderRadius: 999, overflow: 'hidden' }}>
                      <div style={{ width: `${p.pct}%`, height: '100%', background: p.pct === 100 ? '#16a34a' : 'var(--color-accent)', borderRadius: 999, transition: 'width .3s' }} />
                    </div>
                    <span style={{ fontSize: 12, color: 'var(--color-gray-500)', whiteSpace: 'nowrap' }}>{p.pct}%</span>
                  </div>
                </td>
                ...
                <td>
                  <span style={{ fontWeight: p.completedLessons.length === 10 ? 700 : 400, color: p.completedLessons.length === 10 ? '#16a34a' : 'inherit' }}>
                ...
                <td>
                  {p.quizPassed ? (
                    <span style={{ color: '#16a34a', fontWeight: 600 }}>Passed ({p.quizScore}/10)</span>
                  ) : p.quizScore !== null ? (
                    <span style={{ color: '#dc2626', fontSize: 13 }}>Failed ({p.quizScore}/10)</span>
                  ) : (
                    <span style={{ color: 'var(--color-gray-300)' }}>{'\u2014'}</span>
                  )}
                </td>
                ...
                <td>
                  {p.certified ? (
                    <span style={{ color: '#16a34a', fontWeight: 600, fontSize: 13 }}>{'\u2713'} Issued</span>
                  ) : (
                    <span style={{ color: 'var(--color-gray-300)' }}>{'\u2014'}</span>
                  )}
                </td>
                ...
                <td>
                  <span className={`portal-badge ${TT_STATUS_LABELS[p.status].cls}`}>
                        ...
                        <div className="training-report__unit-card">
                          ...
                          <div className="training-report__lesson-list">
                            <div className="training-report__lesson-row">
                              ...
                              <span style={{ fontSize: 13, color: p.scenarioDone ? 'var(--color-navy)' : 'var(--color-gray-400)' }}>
                                Branching Scenario {'\u2014'} The Unexpected Objection
                              </span>
                            </div>
                            <div className="training-report__lesson-row">
                              <span style={{ color: p.quizPassed ? '#16a34a' : 'var(--color-gray-300)', fontSize: 13, lineHeight: 1 }}>
                              ...
                              <span style={{ fontSize: 13, color: p.quizPassed ? 'var(--color-navy)' : 'var(--color-gray-400)' }}>
                                Knowledge Check {p.quizScore !== null ? `(${p.quizScore}/10)` : ''}
                              </span>
                            </div>
                          </div>
                        </div>
