...
function aggregateAdmissibility(rows) {
  ...
  const steps =
    completedLessons.length + (scenarioDone ? 1 : 0) + (quizPassed ? 1 : 0)
...
// --- Trial Testimony helpers ---

// 12 completable steps: lessons 1-10 + scenario + quiz
function aggregateTrialTestimony(rows) {
  ...
  const quizRow = rows.find((r) => r.lesson_id === 'quiz')
  const quizScore = quizRow?.quiz_score?.score ?? null
  const quizPassed = quizScore !== null && quizScore >= 8
  const certified = !!quizRow?.certificate_issued_at
  const certName = quizRow?.certificate_name || ''
  const certDate = quizRow?.certificate_issued_at || null
  ...
  const steps =
    completedLessons.length + (scenarioDone ? 1 : 0) + (quizPassed ? 1 : 0)
  const pct = Math.round((steps / 12) * 100)
...
export default function TrainingReport() {
  ...
  useEffect(() => {
    const load = async () => {
      const [
        ...
        { data: admissibilityProgress },
        { data: reportWritingProgress },
        { data: depositionProgress },
        { data: trialTestimonyProgress },
      ] = await Promise.all([
        supabase
          .from('profiles')
        ...
        supabase
          ...
          .select(
            'user_id, lesson_id, completed, quiz_scores, certificate_name, certificate_issued_at'
          ),
        supabase
          .from('admissibility_progress')
          .select(
            'user_id, lesson_id, completed, quiz_score, certificate_name, certificate_issued_at'
          ),
        supabase
      ...
      const byUserTT = {}
      ;(trialTestimonyProgress || []).forEach((r) => {
        if (!byUserTT[r.user_id]) byUserTT[r.user_id] = []
        byUserTT[r.user_id].push(r)
      })
      ...
      setRows(
        (experts || []).map((exp) => ({
          ...
          trialTestimony: aggregateTrialTestimony(byUserTT[exp.id] || []),
        }))
      )
      setLoading(false)
    }
  ...
  const getStatus = (row) => {
    ...
    if (activeTab === 'trial-testimony') return row.trialTestimony.status
    return row.deposition.status
  }

  const filtered = rows.filter(({ expert, ...rest }) => {
    const name =
      `${expert.first_name || ''} ${expert.last_name || ''} ${expert.email || ''}`.toLowerCase()
  ...
  const midStatLabel =
    activeTab === 'foundations' ? 'Asmt. Passed' : 'Quiz Passed'
    ...
    <div>
      ...
      >
        <button
          ...
          style={tabStyle('foundations')}
        >
          Expert Witness Foundations
        </button>
        <button
          onClick={() => handleTabChange('admissibility')}
          style={tabStyle('admissibility')}
        >
          Standards of Admissibility
        </button>
        <button
          onClick={() => handleTabChange('report-writing')}
          style={tabStyle('report-writing')}
        >
          Report Writing
        </button>
        <button
          onClick={() => handleTabChange('deposition')}
          style={tabStyle('deposition')}
        >
          Deposition
        </button>
        <button
          onClick={() => handleTabChange('trial-testimony')}
          style={tabStyle('trial-testimony')}
        >
          Trial Testimony
        </button>
      </div>
      ...
      <div className="portal-stats" style={{ marginBottom: 24 }}>
        ...
        <div
          className="portal-stat"
          style={{ cursor: 'pointer' }}
          onClick={() => setFilterStatus(midStatKey)}
        >
        ...
        <tbody>
          {filtered.map(({ expert, foundations: p }) => (
            <React.Fragment key={expert.id}>
              <tr
                className={
                  expanded === expert.id ? 'portal-table__row--active' : ''
                }
                style={{ cursor: 'pointer' }}
                onClick={() =>
                  setExpanded(expanded === expert.id ? null : expert.id)
                }
              >
                <td>
                ...
                <td>
                  <span
                    style={{
                      fontWeight:
                        p.lessonsCompleted === TOTAL_LESSONS ? 700 : 400,
                      color:
                ...
                <td>
                  <span
                    style={{
                      fontWeight: p.quizzesPassed === TOTAL_QUIZZES ? 700 : 400,
                      color:
                  ...
                  {p.assessmentPassed ? (
                    <span style={{ color: '#16a34a', fontWeight: 600 }}>
                      ...
                      {p.assessmentScore !== null
                        ? `(${p.assessmentScore}/10)`
                        : ''}
                    </span>
                  ) : p.assessmentScore !== null ? (
                    <span style={{ color: '#dc2626', fontSize: 13 }}>
                      Failed ({p.assessmentScore}/10)
                    </span>
                  ) : (
                ...
                <td>
                  {p.certified ? (
                    <div>
                      ...
                      >
                        {new Date(p.certDate).toLocaleDateString('en-US', {
                          month: 'short',
                          day: 'numeric',
                          year: 'numeric',
                        })}
                ...
                <td>
                  <span
                    className={`portal-badge ${STATUS_LABELS[p.status].cls}`}
                  >
                ...
                <tr>
                  ...
                  >
                    <div className="training-report__detail">
                      <div className="training-report__detail-heading">
                      ...
                      <div className="training-report__unit-grid">
                        {UNITS.map((unit) => {
                          ...
                          return (
                            <div
                              key={unit.id}
                              className="training-report__unit-card"
                            >
                              ...
                              <div className="training-report__lesson-list">
                                {unit.lessons.map((lid) => {
                                  const done =
                                    p.completedLessonIds.includes(lid)
                                  return (
                                    <div
                                      key={lid}
                                      className="training-report__lesson-row"
                                    >
                                      <span
                                        style={{
                                          color: done
                      ...
                      >
                        ...
                        <div>
                          ...
                          >
                            {p.certified
                              ? `Issued to "${p.certName}" on ${new Date(p.certDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}`
                              : 'Not issued'}
                          </span>
                        </div>
                      </div>
            ...
            <React.Fragment key={expert.id}>
              ...
              >
                ...
                <td>
                  <span
                    style={{
                      fontWeight: p.completedLessons.length === 3 ? 700 : 400,
                      color:
                        p.completedLessons.length === 3 ? '#16a34a' : 'inherit',
                    }}
                  >
                ...
                <td>
                  {p.quizPassed ? (
                    <span style={{ color: '#16a34a', fontWeight: 600 }}>
                      Passed ({p.quizScore}/5)
                    </span>
                  ) : p.quizScore !== null ? (
                    <span style={{ color: '#dc2626', fontSize: 13 }}>
                      Failed ({p.quizScore}/5)
                    </span>
                  ) : (
                    <span style={{ color: 'var(--color-gray-300)' }}>
                      {'\u2014'}
                ...
                <td>
                  <span
                    className={`portal-badge ${AD_STATUS_LABELS[p.status].cls}`}
                  >
                        ...
                        <div className="training-report__unit-card">
                          ...
                          <div className="training-report__lesson-list">
                            {['1', '2', '3'].map((lid) => {
                              ...
                              const titles = {
                                1: 'The Evolution of Expert Witness Standards',
                                2: 'Applying Frye and Kelly in Practice',
                                3: 'Daubert and the Modern Framework',
                              }
                              return (
                                <div
                                  key={lid}
            ...
            <React.Fragment key={expert.id}>
              ...
              >
                ...
                <td>
                  <span
                    style={{
                      fontWeight: p.completedLessons.length === 8 ? 700 : 400,
                      color:
                        p.completedLessons.length === 8 ? '#16a34a' : 'inherit',
                    }}
                  >
                ...
                <td>
                  {p.quizPassed ? (
                    <span style={{ color: '#16a34a', fontWeight: 600 }}>
                      Passed ({p.quizScore}/8)
                    </span>
                  ) : p.quizScore !== null ? (
                    <span style={{ color: '#dc2626', fontSize: 13 }}>
                      Failed ({p.quizScore}/8)
                    </span>
                  ) : (
                    <span style={{ color: 'var(--color-gray-300)' }}>
                      {'\u2014'}
                ...
                <td>
                  <span
                    className={`portal-badge ${RW_STATUS_LABELS[p.status].cls}`}
                  >
          ...
          {filtered.map(({ expert, deposition: p }) => (
            <React.Fragment key={expert.id}>
              ...
              >
                ...
                <td>
                  <span
                    className={`portal-badge ${DP_STATUS_LABELS[p.status].cls}`}
                  >
                        ...
                        <div className="training-report__unit-card">
                          ...
                          <div className="training-report__lesson-list">
                            <div className="training-report__lesson-row">
                              ...
                              >
                                Branching Scenario {'\u2014'} The Expert Under
                                Fire
                              </span>
                            </div>
                            <div className="training-report__lesson-row">
...
function TrialTestimonyTable({ filtered, expanded, setExpanded }) {
  return (
    <div className="portal-table-wrap">
      <table className="portal-table training-report-table">
        <thead>
        ...
        <tbody>
          {filtered.map(({ expert, trialTestimony: p }) => (
            <React.Fragment key={expert.id}>
              <tr
                className={
                  expanded === expert.id ? 'portal-table__row--active' : ''
              ...
              >
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: '100%',
                          background:
                            p.pct === 100 ? '#16a34a' : 'var(--color-accent)',
                          borderRadius: 999,
                          transition: 'width .3s',
                        }}
                      />
                ...
                <td>
                  <span
                    style={{
                      fontWeight: p.completedLessons.length === 10 ? 700 : 400,
                      color:
                        p.completedLessons.length === 10
                          ? '#16a34a'
                          : 'inherit',
                    }}
                  >
                ...
                <td>
                  {p.quizPassed ? (
                    <span style={{ color: '#16a34a', fontWeight: 600 }}>
                      Passed ({p.quizScore}/10)
                    </span>
                  ) : p.quizScore !== null ? (
                    <span style={{ color: '#dc2626', fontSize: 13 }}>
                      Failed ({p.quizScore}/10)
                    </span>
                  ) : (
                    <span style={{ color: 'var(--color-gray-300)' }}>
                      {'\u2014'}
                ...
                <td>
                  <span
                    className={`portal-badge ${TT_STATUS_LABELS[p.status].cls}`}
                  >
                    {TT_STATUS_LABELS[p.status].label}
                  </span>
                </td>
                        ...
                        <div className="training-report__unit-card">
                          ...
                          <div className="training-report__lesson-list">
                            <div className="training-report__lesson-row">
                              ...
                              >
                                Branching Scenario {'\u2014'} The Unexpected
                                Objection
                              </span>
                            </div>
                            <div className="training-report__lesson-row">
