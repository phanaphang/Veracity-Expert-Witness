...
function buildProfilePdf(expert, specialties, testimony, documents, jsPDF) {
  ...
  if (expert.rate_review_report)
    y = addField(
      doc,
      y,
      'Rate - Review & Report',
      `$${expert.rate_review_report}/hr`
    )
  if (expert.rate_deposition)
    y = addField(doc, y, 'Rate - Deposition', `$${expert.rate_deposition}/hr`)
  if (expert.rate_trial_testimony)
    y = addField(
      doc,
      y,
      'Rate - Trial Testimony',
      `$${expert.rate_trial_testimony}/hr`
    )
  if (expert.bio) {
    y += 2
    doc.setFont('helvetica', 'bold')
  ...
  // Prior Testimony
  const retainedTotal = testimony.filter(
  ...
  if (retainedTotal > 0) {
    ...
    testimony.forEach((t) => {
      if (t.retained_by && rc[t.retained_by] !== undefined) rc[t.retained_by]++
    })
  ...
  if (testimony.length === 0) {
    ...
    for (const t of testimony) {
      ...
      if (t.retained_by) {
        doc.text(
          `  [${t.retained_by}]`,
          MARGIN + doc.getTextWidth((t.case_name || '') + ' '),
          y
        )
      }
      y += LINE_HEIGHT
      ...
      if (t.date_of_testimony) {
        doc.setTextColor(120)
        doc.text(
          `  ${new Date(t.date_of_testimony).toLocaleDateString()}`,
          MARGIN,
          y
        )
        doc.setTextColor(0)
  ...
  if (documents.length > 0) {
    ...
    for (const d of documents) {
      ...
      if (d.document_type) {
        ...
        doc.text(
          `  (${d.document_type})`,
          MARGIN + doc.getTextWidth(`  •  ${d.file_name} `),
          y
        )
        doc.setTextColor(0)
      }
...
export async function generateExpertPdf(
  expert,
  specialties,
  testimony,
  documents,
  ...
  const profileBytes = buildProfilePdf(
    expert,
    specialties,
    testimony,
    documents,
  ...
  let finalBytes
  if (documents.length > 0) {
    finalBytes = await mergeDocuments(
      profileBytes,
      documents,
      supabase,
      PDFDocument
  ...
  const name = expert.first_name
    ? `${expert.first_name}_${expert.last_name || ''}`
        .trim()
        .replace(/\s+/g, '_')
    : 'expert'
