  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase
        ...
        .select('specialties(id, name, parent_id)')
        .eq('expert_id', id),
      supabase
      ...
      .then(([prof, specs, test, docs, parents]) => {
        if (cancelled) return
        const specObjs =
          specs.data?.map((s) => s.specialties).filter(Boolean) || []
        setExpert(prof.data)
  ...
  if (!expert)
    return (
      <div className="portal-empty">
        <p className="portal-empty__text">Expert not found</p>
      </div>
    )
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
        <span
          className={`portal-badge portal-badge--${expert.profile_status || 'pending'}`}
        >
          {expert.profile_status || 'pending review'}
        </span>
        {expert.onboarded_at && (
          <span className="portal-badge portal-badge--accepted">onboarded</span>
        )}
      </div>
      ...
      <div className="portal-card">
        ...
        <div className="portal-list-item__row">
          ...
          <div>
            <strong>Phone:</strong> {formatPhone(expert.phone)}
          </div>
        </div>
        {(expert.rate_review_report ||
          expert.rate_deposition ||
          expert.rate_trial_testimony) && (
          <div style={{ marginTop: 8 }}>
            ...
            >
              {expert.rate_review_report && (
                <span>Review & Report: ${expert.rate_review_report}/hr</span>
              )}
              {expert.rate_deposition && (
                <span>Deposition: ${expert.rate_deposition}/hr</span>
              )}
              {expert.rate_trial_testimony && (
                <span>Trial Testimony: ${expert.rate_trial_testimony}/hr</span>
              )}
            </div>
          </div>
        )}
        ...
        <div className="portal-card">
          ...
          {(() => {
            const grouped = parentSpecialties
              .map((parent) => ({
                parent,
                subs: specialtyObjects.filter((s) => s.parent_id === parent.id),
              }))
                ...
                {legacy.length > 0 && (
                  <div
                    style={{
                      ...
                      flexWrap: 'wrap',
                      gap: 6,
                      marginBottom: grouped.length > 0 ? 8 : 0,
                    }}
                  >
                    {legacy.map((s) => (
                      <span
      ...
      <div className="portal-card">
        <h2 className="portal-card__title">
          Prior Expert Testimony ({testimony.length})
          {testimony.length > 0 &&
            (() => {
        ...
        {testimony.length === 0 ? (
          ...
          testimony.map((test) => (
            ...
            >
              ...
              {test.date_of_testimony && (
                <p
                  style={{ fontSize: '0.8rem', color: 'var(--color-gray-400)' }}
                >
                  {new Date(test.date_of_testimony).toLocaleDateString()}
                </p>
              )}
            </div>
          ))
