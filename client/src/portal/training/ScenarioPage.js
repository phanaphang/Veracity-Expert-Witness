  ...
  return (
    <div className="training-scenario">
      <div className="portal-page__header">
        <div className="training-lesson__unit-badge">
          {unit.title} · Branching Scenario
        </div>
      ...
      {previousChoice && !selected && (
        <div className="portal-alert" style={{ marginBottom: 16 }}>
          You previously chose option{' '}
          {scenario.choices.findIndex((c) => c.id === previousChoice) + 1}. You
          can review your choice below or try a different option.
        </div>
      )}
      ...
      {activeChoice && (
        <div className="training-scenario__result">
          <div
            className={`training-scenario__consequence${activeChoice.correct ? ' training-scenario__consequence--correct' : ' training-scenario__consequence--wrong'}`}
          >
