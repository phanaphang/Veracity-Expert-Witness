...
// Renders the rich content sections defined in reportWritingData.js
...
export default function ReportWritingLessonPage({ onProgressUpdate }) {
  ...
  const handleMarkComplete = async () => {
    ...
    try {
      ...
      if (nextLessonId) {
        navigate(`/training/report-writing/lesson/${nextLessonId}`);
      } else {
        // Last lesson - go to scenario
        navigate('/training/report-writing/scenario');
      }
    } catch (e) {
  ...
  if (!lesson) {
    return (
      <div>
        ...
        <Link to="/training/report-writing" className="btn btn--secondary" style={{ marginTop: 16 }}>
          Back to Training
        </Link>
      </div>
    );
      ...
      <div className="training-lesson__footer">
        ...
        {alreadyCompleted ? (
          <div className="training-lesson__already-done">
            ...
            {nextLessonId && (
              <Link
                to={`/training/report-writing/lesson/${nextLessonId}`}
