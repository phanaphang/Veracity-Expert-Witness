...
import { QUIZ_DATA, PASS_SCORE, TOTAL_QUESTIONS } from './reportWritingData'

export default function ReportWritingQuizPage({ onProgressUpdate }) {
    ...
    if (isPassing || attempts >= 2) {
      ...
      try {
        ...
        if (existing) {
          ...
          await supabase.from('report_writing_progress').insert({
            user_id: user.id,
            lesson_id: 'quiz',
            ...patch,
          })
        ...
        if (isPassing) {
          ...
          if (missed === 0) {
            setTimeout(
              () => navigate('/training/report-writing/certificate'),
              1800
            )
          }
        }
    ...
    <div className="training-quiz">
      ...
      {alreadyPassed && !submitted && (
        ...
        >
          ...
          or{' '}
          <Link to="/training/report-writing/certificate">
      ...
      {outOfRetries && (
        <div className="training-quiz__result training-quiz__result--fail">
          ...
          <div className="training-quiz__result-actions">
            <Link
              to="/training/report-writing/lesson/1"
              className="btn btn--primary"
            >
              Review Lessons
            </Link>
      ...
      <div style={{ marginTop: 24 }}>
        <Link to="/training/report-writing" className="training-lesson__back">
          ← Training Home
        </Link>
      </div>
    </div>
