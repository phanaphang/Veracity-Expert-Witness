    ...
    <div className="training-quiz">
      <div className="portal-page__header">
        ...
        <p className="portal-page__subtitle">
          {TOTAL} questions · Pass score: {PASS_SCORE}/{TOTAL} (80%) · Covers
          all 4 units
        </p>
      </div>
          ...
          <div className="training-assessment__breakdown">
            {unitBreakdown.map(({ unit, total, correct }) => (
              <div key={unit.id} className="training-assessment__unit-row">
                <div className="training-assessment__unit-name">
                  {unit.title}
                </div>
                <div
                  className={`training-assessment__unit-score${correct === total ? ' training-assessment__unit-score--full' : ''}`}
                >
                  {correct}/{total}
                </div>
                <Link
                  to={`/training/lesson/${unit.lessons[0]}`}
                  className="training-assessment__review-link"
                >
                  Review →
                </Link>
              </div>
        ...
        <>
            ...
            return (
              <div key={q.id} className="training-quiz__question portal-card">
                <div className="training-quiz__q-meta">
                  <span className="training-quiz__q-number">
                    Question {qi + 1}
                  </span>
          ...
          {failed && canRetry && (
            <div className="training-assessment__breakdown portal-card">
              <h3 style={{ margin: '0 0 16px', color: 'var(--color-navy)' }}>
                Score by Unit
              </h3>
              {unitBreakdown.map(({ unit, total, correct }) => (
                <div key={unit.id} className="training-assessment__unit-row">
          ...
          {!submitted ? (
            ...
            <div className="training-quiz__retry-bar">
              <div>
                ...
                You need {PASS_SCORE} to pass. Review the explanations and unit
                scores above, then try once more.
              </div>
