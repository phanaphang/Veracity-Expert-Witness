    ...
    for (const unit of UNITS) {
      for (const lid of unit.lessons) {
        if (!completedLessons.includes(lid)) {
          return {
            type: 'lesson',
            id: lid,
            label: `Continue: Lesson ${lid}`,
            to: `/training/lesson/${lid}`,
          }
        }
      }
      ...
      if (!quizPassed) {
        return {
          type: 'quiz',
          id: unit.quizId,
          label: `Take Knowledge Check: ${unit.title}`,
  ...
  return (
    <div>
      <div className="portal-page__header">
        <h1 className="portal-page__title">Expert Witness Foundations</h1>
        <p className="portal-page__subtitle">
          Welcome{profile?.first_name ? `, ${profile.first_name}` : ''}.
          Complete all 4 units to earn your certificate.
        </p>
      </div>
      ...
      <div className="portal-card training-progress-card">
        ...
        <div className="training-progress-card__meta">
          {completedLessons.length} of {TOTAL_LESSONS} lessons complete · ~60
          min total
        </div>
        {!loading && (
          <Link
      ...
      <div className="training-units">
        {UNITS.map((unit, ui) => {
          ...
          const accessible =
            isAdminOrStaff ||
            !prevUnit ||
            (passedQuizzes[prevUnit.quizId] || 0) >= 2
              ...
              <ul className="training-unit-card__lessons">
                {unit.lessons.map((lid) => {
                  ...
                  return (
                    <li
                      key={lid}
                      className={`training-unit-card__lesson${done ? ' training-unit-card__lesson--done' : ''}`}
                    >
                      {done ? (
                        <svg
                          viewBox="0 0 16 16"
        ...
        <div className="training-resources-card__body">
          <div>
            ...
            <p className="training-resources-card__sub">
              5 reference guides — available anytime
            </p>
          </div>
