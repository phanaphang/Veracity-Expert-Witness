...
export default function Profile() {
  ...
  const toggleParent = (parentId) => {
    setExpandedParents((prev) => {
      ...
      if (next.has(parentId)) {
        ...
        const subIds = allSpecialties
          .filter((s) => s.parent_id === parentId)
          .map((s) => s.id)
        setSelectedSpecialties((sel) =>
  ...
  const sanitizeTag = (value) =>
    value.replace(/[^a-zA-Z0-9\s\-&().,']/g, '').slice(0, TAG_MAX_LENGTH)
    ...
    try {
      ...
      for (const item of testimony) {
        const data = {
          ...
          date_of_testimony: item.date_of_testimony || null,
          topic: item.topic || '',
    ...
    <div>
      ...
      <form id="profile-form" onSubmit={handleSubmit}>
        ...
        <div className="portal-card">
          ...
          <div className="portal-field">
            <label className="portal-field__label" htmlFor="profile-email">
              Email
            </label>
            <input
              id="profile-email"
          ...
          <div className="portal-field">
            <label className="portal-field__label" htmlFor="profile-phone">
              Phone
            </label>
            <input
              id="profile-phone"
          ...
          <div className="portal-field">
            <label className="portal-field__label" htmlFor="profile-bio">
              Bio
            </label>
            <textarea
              id="profile-bio"
        ...
        <div className="portal-card">
          ...
          >
            ...
            >
              Add specific areas of expertise (e.g. &quot;Soil and groundwater
              contamination&quot;, &quot;PFAS remediation&quot;). Press Enter or
              comma to add.
            </p>
            <div
              style={{
            ...
            {editing && (
              <>
                ...
                {tags.length >= TAG_MAX_COUNT && (
                  ...
                  >
                    Maximum of {TAG_MAX_COUNT} tags reached
                  </span>
                )}
                <div
                  style={{
                ...
                >
                  <span>
                    {tags.length}/{TAG_MAX_COUNT} added
                  </span>
                  <span>
                    {tagInput.length}/{TAG_MAX_LENGTH} characters
                  </span>
                </div>
              </>
            )}
        ...
        <div className="portal-card">
          <h2 className="portal-card__title">
            ...
            {testimony.length > 0 &&
              (() => {
                const total = testimony.filter(
                  (t) => t.retained_by && t.retained_by !== ''
                ).length
                if (total === 0) return null
                const counts = { plaintiff: 0, defendant: 0, other: 0 }
                testimony.forEach((t) => {
                  if (t.retained_by && counts[t.retained_by] !== undefined)
                    counts[t.retained_by]++
                })
                ...
                return (
                  ...
                  >
                    Plaintiff {pct(counts.plaintiff)}% | Defendant{' '}
                    {pct(counts.defendant)}% | Other {pct(counts.other)}%
                  </span>
                )
              })()}
          </h2>
          {testimony.map((test, i) => (
            <div key={i} className="portal-list-item">
              {editing && (
                <button
                  type="button"
                  className="portal-list-item__remove"
                  onClick={() =>
                    removeItem(testimony, setTestimony, i, 'prior_testimony')
                  }
                >
                  Remove
                </button>
              ...
              <div className="portal-list-item__row">
                <div className="portal-field">
                  ...
                  <input
                    className="portal-field__input"
                    value={test.case_name}
                    onChange={(e) => {
                      const t = [...testimony]
                      t[i].case_name = e.target.value
                      setTestimony(t)
                    }}
                    maxLength={150}
                    readOnly={!editing}
              ...
              <div className="portal-list-item__row">
                <div className="portal-field">
                  ...
                  <input
                    className="portal-field__input"
                    value={test.jurisdiction || ''}
                    onChange={(e) => {
                      const t = [...testimony]
                      t[i].jurisdiction = e.target.value
                      setTestimony(t)
                    }}
                    maxLength={100}
                    readOnly={!editing}
                ...
                <div className="portal-field">
                  ...
                  <input
                    className="portal-field__input"
                    type="date"
                    value={test.date_of_testimony || ''}
                    onChange={(e) => {
                      const t = [...testimony]
                      t[i].date_of_testimony = e.target.value
                      setTestimony(t)
                    }}
                    readOnly={!editing}
                  />
              ...
              <div className="portal-list-item__row">
                ...
                <div className="portal-field">
                  ...
                  <select
                    ...
                    value={test.retained_by || ''}
                    onChange={(e) => {
                      const t = [...testimony]
                      t[i].retained_by = e.target.value
                      setTestimony(t)
                    }}
                    disabled={!editing}
                  >
        ...
        <div className="portal-card">
          ...
          <div className="portal-list-item__row">
            <div className="portal-field">
              <label
                className="portal-field__label"
                htmlFor="profile-rate-review"
              >
                Review & Report Rate ($/hr)
              </label>
              <input
                id="profile-rate-review"
                className="portal-field__input"
                name="rate_review_report"
                type="number"
                value={form.rate_review_report}
                onChange={handleChange}
                placeholder="e.g. 500"
                min={0}
                max={9999}
            ...
            <div className="portal-field">
              <label
                className="portal-field__label"
                htmlFor="profile-rate-deposition"
              >
                Deposition Rate ($/hr)
              </label>
              <input
                id="profile-rate-deposition"
                className="portal-field__input"
                name="rate_deposition"
                type="number"
                value={form.rate_deposition}
                onChange={handleChange}
                placeholder="e.g. 600"
                min={0}
                max={9999}
            ...
            <div className="portal-field">
              <label
                className="portal-field__label"
                htmlFor="profile-rate-trial"
              >
                Trial Testimony Rate ($/hr)
              </label>
              <input
                id="profile-rate-trial"
                className="portal-field__input"
                name="rate_trial_testimony"
                type="number"
                value={form.rate_trial_testimony}
                onChange={handleChange}
                placeholder="e.g. 750"
                min={0}
                max={9999}
            ...
            {cvDocs.map((doc) => (
              <div key={doc.id} className="portal-doc-card">
                ...
                <div className="portal-doc-card__actions">
                  <button
                    className="portal-doc-card__btn"
                    onClick={() => handleCvDownload(doc)}
                    title="Download"
